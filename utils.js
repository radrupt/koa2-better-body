 ...
 * @api private
 */
utils.parseBody = async function parseBody (ctx, options, next) { /* eslint complexity: [2, 12] */
    cons
  ...
  if (custom && custom.length && ctx.request.is(custom)) {
    await options.handler.call(ctx, ctx, options, next)
    return  next
  }
